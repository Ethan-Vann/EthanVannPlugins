...
public class GroundObjectPackets
{
	@Inject
	Client client;
	@Inject
	...
	@SneakyThrows
	public void queueGroundObjectAction(int actionFieldNo, int objectId, int worldPointX, int worldPointY,
								   boolean ctrlDown) {
	...
	@SneakyThrows
	public void queueGroundObjectAction(GroundObject object, boolean ctrlDown, String... actionlist) {
		...
		queueGroundObjectAction(num, object.getId(), wp.getX(), wp.getY(), ctrlDown);
	}
	public void queueWidgetOnGroundObject(int objectId, int worldPointX, int worldPointY, int sourceSlot,
	...
	public void queueWidgetOnGroundObject(Widget widget, TileObject object) {
		Point p;
		if (object instanceof GameObject) {
			GameObject gameObject = (GameObject) object;
			p = gameObject.getSceneMinLocation();
		...
		LocalPoint lp = new LocalPoint(p.getX(), p.getY());
		WorldPoint wp = WorldPoint.fromScene(client, lp.getX(), lp.getY(), object.getPlane());
		queueWidgetOnGroundObject(object.getId(), wp.getX(), wp.getY(), widget.getIndex(),
				widget.getItemId(),
				widget.getId(),
				false);
	}
